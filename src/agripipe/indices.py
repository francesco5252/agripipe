...
def compute_agronomic_indices(df: pd.DataFrame, knowledge: dict) -> pd.DataFrame:
    ...
    if date_col and field_col:
        ...
        if rain_col:
            ...
            df["drought_7d_score"] = df.groupby(field_col)["daily_wb"].transform(lambda x: x.rolling(7, min_periods=1).sum())
    ...
    if yield_col and n_col:
        ...
        df["n_efficiency"] = df[yield_col] / (df[n_col] + 1.0) # +1.0 per evitare divisioni per zero
