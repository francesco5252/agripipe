...
if uploaded_file is not None:
    ...
    if st.button("🚀 Avvia Ottimizzazione Personalizzata"):
        with st.spinner("Applicando regole territoriali specifiche..."):
            ...
            m2.metric(
                "Anomalie Corrette", len(df_raw) - len(df_clean) + df_clean.isna().sum().sum()
            )
            ...
            st.markdown(
                "Confronto tra i dati **Grezzi** (input) e quelli **Ottimizzati** (output) secondo le regole territoriali."
            )
            ...
            for col in [c for c in ["yield", "temp", "ph", "humidity"] if c in numeric_cols]:
                with st.expander(f"Dettaglio Colonna: {col.upper()}", expanded=(col == "yield")):
                    ...
                    sns.boxplot(
                        data=data_cmp, x="Stato", y=col, ax=ax1, palette=["#ff9999", "#66b3ff"]
                    )
                    ...
                    sns.kdeplot(
                        df_clean[col], fill=True, label="Ottimizzato", ax=ax2, color="#66b3ff"
                    )
